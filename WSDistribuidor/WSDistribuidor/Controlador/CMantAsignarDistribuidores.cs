...
{
    public class CMantAsignarDistribuidores
    {
        public List<EMantenimiento> MantAsignarDistribuidores(SqlConnection con, Int32 post, String ruc, Int32 usuario, Int32 user)
        {
            List<EMantenimiento> lEMantenimiento = null;
            SqlCommand cmd = new SqlCommand("ASP_MANT_ASIGNAR_DISTRIBUIDOR", con);
            cmd.CommandType = CommandType.StoredProcedure;

            cmd.Parameters.AddWithValue("@post", SqlDbType.Int).Value = post;
            cmd.Parameters.AddWithValue("@ruc", SqlDbType.VarChar).Value = ruc;
            cmd.Parameters.AddWithValue("@usuario", SqlDbType.Int).Value = usuario;
