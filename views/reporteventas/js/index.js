$(function () {
  ...
  $("#btnfecha").on("click", function () {
    ...
    $.ajax({
      type: "POST",
      url: "/portaldistribuidor/reporteventas/cargar_combos",
      data: { fechainicio: fechainicio, fechafin: fechafin },
      beforeSend: function () {
        $("#btnfecha").attr("disabled", "disabled");
        $("#btnfecha").html(
          "<span class='spinner-border spinner-border-sm'></span> \
                            <span class='ml-25 align-middle'>Cargando datos...</span>"
        );
      },
      success: function (res) {
        $("#btnfecha").prop("disabled", false);
        $("#btnfecha").html("Cargar datos");

        $("#departamento").html("");
        $("#departamento").append(res.departamento);
        $("#distrito").html("");
        $("#distrito").append(res.distrito);
        $("#ruc").html("");
        $("#ruc").append(res.distribuidor);
      },
    });
  });
  ...
  $(".departamento").change(function () {
    arrdepartamento = $(this).val();
  });

  $(".distrito").change(function () {
    arrdistrito = $(this).val();
  });
  ...
  $("#btnfiltrofecha").on("click", function () {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: "POST",
          url: "/portaldistribuidor/reporteventas/filtro",
          data: {
            post: post,
            finicio: finicio,
            ffin: ffin,
  ...
  $("#btnaceptar").on("click", function () {
    ...
    let departamento = arrdepartamento; //array de departamentos
    let distrito = arrdistrito; //array de distrito
